...
class BertBase(BertABC):
    def __init__(
        ...
        # At this point, self.device is set; you can now move the model to this device when needed.
    ...
    def predict_with_model(
        ...
        if torch.cuda.is_available():
            model.cuda()
        # On déplace le modèle sur self.device, qu'il s'agisse de CUDA, MPS ou CPU
        model.to(self.device)
